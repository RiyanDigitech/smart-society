...
const AnnoucementTable: React.FC = () => {
  ...
  const { useFetchAnnoucement} = AnnoucementService();
  
 // const deleteMutation = useDeleteAnnouncementById();
...
//   };
  ...
  const columns: ColumnsType<AnnoucementData> = [
   ...
   
    {
      ...
      render: (body: string) =>
        body ? (
          <span className="text-[#4b5563]">{body.length > 30 ? `${body.slice(0, 30)}...` : body}</span>
        ) : (
          "-"
        ),
    ...
    
    {
    ...
    },
     {
      ...
      render: (announcementStatus) => <span className="text-[#4b5563] ">{announcementStatus}</span>,
    },
